...
while True:
    ...
    match event:
        ...
        case sg.WIN_CLOSED:
            break
        case 'D':
            robot.animation(qseq, np.roll(
                qseq, qseq.shape[1]//2), qseq, np.roll(qseq, qseq.shape[1]//2), 3)
            robot.set_position((0, 7), (True, True, True, True))

        case 'R':
            robot.animation(qseq[::, ::-1], np.roll(
                qseq[::, ::-1], qseq.shape[1]//2), qseq[::, ::-1], np.roll(qseq[::, ::-1], qseq.shape[1]//2), 3)
            robot.set_position((0, 7), (True, True, True, True))
        case _:
